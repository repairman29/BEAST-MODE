/**
 * BEAST MODE
 * Quality Intelligence & Marketplace Platform
 *
...
const BEAST_MODE = {
    name: 'BEAST MODE',
    version: '1.0.0',
    description: 'Quality Intelligence & Marketplace Platform',
...
class BeastMode {
    constructor(options = {}) {
        this.options = {
            ...
            marketplace: true,
            intelligence: true,
            apiKey: options.apiKey || process.env.BEAST_MODE_API_KEY,
            ...options
        };
    ...
    const { LicenseValidator } = require('./licensing/license-validator');
    ...
    async initialize() {
        ...
        try {
            // Validate license/subscription first
            const licenseResult = await this.licenseValidator.validate();
            this.subscriptionTier = licenseResult.tier;
            this.subscription = licenseResult.subscription;
            
            log.info(`📦 Subscription tier: ${this.subscriptionTier}`);
            
            // Check API limits
            const apiLimit = await this.licenseValidator.checkApiLimit();
            if (!apiLimit.allowed) {
                log.warn(`⚠️  API call limit reached (${apiLimit.used}/${apiLimit.limit})`);
            }
            // Initialize core components
            ...
            // Initialize Janitor System (The AI Janitor)
            // Check if user has access to Day 2 Operations features
            if (this.options.janitor !== false) {
                const hasDay2Ops = this.licenseValidator.checkFeature('day2-operations');
                
                if (!hasDay2Ops && this.subscriptionTier === 'free') {
                    log.warn('⚠️  Day 2 Operations requires Developer tier or higher');
                    log.info('💡 Upgrade at https://beast-mode.dev/pricing to unlock Day 2 Operations');
                    // Don't initialize janitor for free tier
                } else {
                    const { detectBrand } = require('./brand-config');
                    const brand = detectBrand(this.options);
                    
                    if (brand === 'sentinel' || this.options.sentinel) {
                        // Enterprise: Use Sentinel
                        const { Sentinel } = require('./sentinel');
                        this.sentinel = new Sentinel(this.options.sentinel || this.options.janitor || {});
                        await this.sentinel.initialize();
                        this.janitor = this.sentinel.janitor; // Expose janitor for compatibility
                        this.components.set('sentinel', this.sentinel);
                        this.components.set('janitor', this.janitor);
                        log.info('🛡️ SENTINEL mode enabled - Enterprise governance');
                    } else {
                        // Community: Use BEAST MODE Janitor
                        const { Janitor } = require('./janitor');
                        this.janitor = new Janitor(this.options.janitor || {});
                        await this.janitor.initialize();
                        this.components.set('janitor', this.janitor);
                        log.info('🎸 BEAST MODE mode enabled - Community vibe');
                    }
                    
                    // Integrate with other BEAST MODE systems
                    if (this.janitor && this.janitor.integrateWithBeastMode) {
                        await this.janitor.integrateWithBeastMode(this);
                    }
                }
            }
    ...
    async predict(options = {}) {
        ...
        if (!this.hasFeature('advanced-analytics') && this.subscriptionTier === 'free') {
            throw new Error('Predictive analytics requires Developer tier or higher. Upgrade at https://beast-mode.dev/pricing');
        }
    ...
    async browseMarketplace(options = {}) {
        ...
        // Marketplace is available to all tiers
     ...
     */
    hasFeature(feature) {
        return this.licenseValidator.checkFeature(feature);
    }
     ...
     */
    getSubscription() {
        return this.subscription || this.licenseValidator.getSubscription();
    }
     ...
     */
    async checkApiLimit() {
        return await this.licenseValidator.checkApiLimit();
    }
