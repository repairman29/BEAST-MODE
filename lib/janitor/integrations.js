...
class JanitorIntegrations {
    ...
    async integrateAll(beastMode) {
        ...
        // Try to integrate with Daisy Chain
        try {
            await this.integrateWithDaisyChain(beastMode);
        } catch (error) {
            log.debug('Daisy Chain not available');
        }
     ...
     */
    async integrateWithDaisyChain(beastMode) {
        try {
            const { JanitorDaisyChainIntegration } = require('./daisyChainIntegration');
            this.janitor.daisyChainIntegration = new JanitorDaisyChainIntegration(this.janitor);
            await this.janitor.daisyChainIntegration.initialize();
            
            log.info('✅ Integrated with Daisy Chain');
        } catch (error) {
            log.debug('Daisy Chain integration not available:', error.message);
        }
    }
}
