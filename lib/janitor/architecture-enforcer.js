...
`;
    ...
    async checkFile(filePath, content) {
        ...
        // Skip architecture rules route - it documents // SECURITY: // SECURITY: // SECURITY: eval() disabled
// eval() disabled
            ...
            case 'database-in-frontend':
                ...
                fixed = fixed.replace(
                    ...
                    (match, fullMatch, method, offset) => {
                        ...
                        // Check if we're in an API route - skip those
                        if (filePath.includes('/api/') || filePath.includes('/routes/')) {
                            return match; // Don't modify API routes
                        }
