    ...
    try {
      ...
      if (redisUrl) {
        if (redisUrl.startsWith('redis://') || redisUrl.startsWith('rediss://')) {
          ...
          this.l2Client = new Redis({ url: redisUrl, token: process.env.UPSTASH_REDIS_REST_TOKEN });
        }
        this.l2Available = true;
        log.info('✅ L2 cache (Redis) initialized');
      } else {
        log.warn('⚠️  Redis URL not found, L2 cache disabled');
        this.l2Available = false;
      }
    } catch (error) {
      log.warn('⚠️  L2 cache (Redis) not available:', error.message);
      this.l2Available = false;
    }
  }
