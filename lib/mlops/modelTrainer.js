...
class ModelTrainer {
  ...
  async trainCodeQualityModel(options = {}) {
    ...
    const {
      minExamples = 30, // Lowered for initial training (was 100)
      testSplit = 0.2,
      validationSplit = 0.1
    } = options;
    ...
    // 2. Filter for code quality examples (or use all if we have quality predictions)
    let codeQualityExamples = dataset.dataset.filter(ex => 
      ex.predictionType === 'code-quality' || 
      ex.serviceName === 'code-roach' ||
      ex.source === 'github' ||
      (ex.predictionType === 'quality' && ex.serviceName === 'beast-mode') // Include quality predictions
    );

    // If we don't have enough code-quality specific examples, use all quality examples
    if (codeQualityExamples.length < minExamples) {
      log.warn(`⚠️  Only ${codeQualityExamples.length} code-quality examples, using all quality examples`);
      const allQualityExamples = dataset.dataset.filter(ex => 
        ex.predictionType === 'quality' || ex.predictionType === 'code-quality'
      );
      if (allQualityExamples.length >= minExamples) {
        codeQualityExamples = allQualityExamples;
      }
    }

    if (codeQualityExamples.length < minExamples) {
      throw new Error(`Not enough training examples: ${codeQualityExamples.length} < ${minExamples}`);
    }
  ...
  trainSimpleModel(trainX, trainY, valX, valY) {
    ...
    let bias = 0; // Changed to let since we update it
