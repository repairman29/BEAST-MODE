...
`;
      ...
      // Fallback for unknown languages - reuse variables declared above
      return `name: CI
...
jobs:
  build:
    ...
    - name: Install dependencies
      run: ${installCmd}
    - name: Build
      run: ${scripts && scripts.build ? buildCmd : 'echo "Build step not configured"'}
      continue-on-error: true
${hasTests && scripts && scripts.test ? `    - name: Run tests
      run: ${testCmd}
      continue-on-error: true
...
`;
    ...
    // README template
      this.templates.set('readme', (context) => {
        ...
        const repoName = repoParts[1] || repoParts[0] || 'unknown';
        // Prioritize primaryLanguage, then language, then first from languages array, fallback to JavaScript for Node.js projects
        const displayLanguage = primaryLanguage || language || (languages && languages.length > 0 && languages[0]) || (packageManager ? 'JavaScript' : 'Unknown');
