...
jobs:
  build:
    ...
    - name: Install dependencies
      run: ${installCmd}
    - name: Build
      run: ${scripts && scripts.build ? (packageManager === 'yarn' ? 'yarn build' : packageManager === 'pnpm' ? 'pnpm build' : 'npm run build') : 'echo "Build step not configured"'}
      continue-on-error: true
    ${hasTests && scripts && scripts.test ? `- name: Run tests
      run: ${packageManager === 'yarn' ? 'yarn test' : packageManager === 'pnpm' ? 'pnpm test' : 'npm test'}
      continue-on-error: true
` : ''}
`;
    });
