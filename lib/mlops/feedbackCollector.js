...
class FeedbackCollector {
  ...
  async recordOutcome(predictionId, actualValue, context = {}, retryCount = 0) {
    ...
    try {
      ...
      // Also write to ml_feedback table for tracking
      try {
        const { data: feedbackEntry, error: fbError } = await this.supabase
          .from('ml_feedback')
          .insert({
            prediction_id: predictionId,
            service_name: prediction.service_name,
            feedback_type: context.feedbackType || 'system',
            feedback_score: actualValue,
            feedback_text: context.feedbackText || null,
            user_id: context.userId || null,
            metadata: {
              ...context,
              source: context.source || 'recordOutcome',
        ...
        if (fbError && fbError.code !== '23505') { // 23505 = duplicate key (ignore)
          log.warn(`⚠️  Failed to write to ml_feedback: ${fbError.message}`);
        } else if (feedbackEntry) {
          log.debug(`✅ Wrote feedback entry: ${feedbackEntry.id}`);
        }
      } catch (error) {
        // Non-critical - feedback table write is optional
        log.debug('[Feedback Collector] Feedback table write failed:', error.message);
      }
