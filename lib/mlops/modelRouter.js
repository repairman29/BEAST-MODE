...
class ModelRouter {
  constructor() {
    ...
    this.cache = new LLMCache({ enabled: true, ttl: 3600000 }); // 1 hour cache
  }
  ...
  async getCustomModel(modelId, userId) {
    ...
    try {
      ...
      if (!apiKey && userId) {
        try {
          // For OpenAI-compatible endpoints, try to get OpenAI key
  ...
  async routeToCustomModel(modelId, request, userId) {
    ...
    try {
      ...
      if (error.response) {
        ...
        if (status === 401) {
          errorMessage = `Custom model authentication failed: Check your API key`;
        } else if (status === 404) {
          errorMessage = `Custom model endpoint not found: Check your endpoint URL`;
        } else if (status >= 500) {
          errorMessage = `Custom model server error: The model endpoint is having issues`;
        }
      } else if (error.code === 'ECONNREFUSED') {
        errorMessage = `Custom model connection refused: Check if endpoint is accessible`;
      } else if (error.code === 'ETIMEDOUT') {
        errorMessage = `Custom model timeout: The endpoint took too long to respond`;
      }
  ...
  async route(modelId, request, userId) {
    ...
    if (modelId.startsWith('custom:')) {
      ...
      if (!canUseProvider) {
        throw new Error('Provider models (OpenAI, Anthropic, etc.) are only available for paid tier users. Free tier users must use custom models. Please upgrade your subscription or register a custom model.');
      }
