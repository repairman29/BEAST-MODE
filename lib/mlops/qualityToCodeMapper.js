...
class QualityToCodeMapper {
  ...
  identifyRecommendationType(recommendation) {
    ...
    // Check for README first (most specific)
    if (action.includes('readme') || action.includes('comprehensive readme') || action.includes('create readme') || action.includes('repository description')) {
      return 'add_readme';
    }
    // Check for CI/CD
    if (action.includes('ci/cd') || action.includes('ci cd') || action.includes('pipeline') || action.includes('establish ci') || action.includes('establish cicd') || action.includes('cicd foundation')) {
      return 'add_ci_cd';
    }
    // Check for tests (broad matching)
    if (action.includes('test') || action.includes('coverage') || action.includes('testing foundation') || action.includes('establish testing') || action.includes('enhance test')) {
      return 'add_tests';
    }
  ...
  mapTestsRecommendation(recommendation, files, repoContext) {
    ...
    // If no files provided, generate a basic test setup
