...
/**
 * Generate BEAST MODE Brand Assets with AI (Direct OpenAI SDK)
 * 
 * Generates logos directly using OpenAI SDK (no API route needed)
 */
...
async function generateAllAssets() {
  ...
  if (!apiKey) {
    ...
    console.log('   3. Or use user API key from Supabase\n');
    process.exit(1);
  }
  ...
  for (let i = 0; i < brandAssets.length; i++) {
    ...
    // Wait between requests to avoid rate limits (DALL-E 3 has rate limits)
