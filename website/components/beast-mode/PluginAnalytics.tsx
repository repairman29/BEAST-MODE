      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="bg-slate-900/90 border-slate-800">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-bold text-white">{analytics.totalExecutions.toLocaleString()}</div>
        ...
        <Card className="bg-slate-900/90 border-slate-800">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-bold text-green-400">{analytics.successRate.toFixed(1)}%</div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-slate-400 text-sm">
                        {plugin.executions} execution{plugin.executions !== 1 ? 's' : ''}
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className={`font-semibold ${
                      plugin.successRate >= 95 ? 'text-green-400' :
                      plugin.successRate >= 80 ? 'text-yellow-400' :
                      'text-red-400'
                    }`}>
          ...
          <CardContent>
            <div className="space-y-2">
              {analytics.usageByDay.slice(-14).map((day) => {
                const maxExecutions = Math.max(...analytics.usageByDay.map(d => d.executions));
                const barWidth = maxExecutions > 0 ? (day.executions / maxExecutions) * 100 : 0;
                  ...
                  <div key={day.date} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="text-slate-400">
                        {day.executions} exec • {day.successes} ✓ • {day.failures} ✗
                      </div>
                    </div>
                    <div className="flex gap-1 h-4">
                      ...
                      <div
                        ...
                        style={{ width: `${maxExecutions > 0 ? (day.successes / maxExecutions) * 100 : 0}%` }}
                        title={`${day.successes} successes`}
                      />
                      <div
                        className="bg-red-500 rounded"
                        style={{ width: `${maxExecutions > 0 ? (day.failures / maxExecutions) * 100 : 0}%` }}
