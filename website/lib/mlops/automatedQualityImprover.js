    ...
    try {
      ...
      try {
        ...
        const possiblePaths = [
          ...
          path.join(workspaceRoot, repoName.replace('smuggler-', '')),
          // Handle special cases
          repoName === 'BEAST-MODE' ? path.join(workspaceRoot, 'BEAST-MODE-PRODUCT') : null,
          repoName === 'daisy-chain' ? path.join(workspaceRoot, 'smuggler-daisy-chain') : null,
          repoName === 'oracle' ? path.join(workspaceRoot, 'smuggler-oracle') : null,
          repoName === 'code-roach' ? path.join(workspaceRoot, 'smuggler-code-roach') : null,
          repoName === 'ai-gm-service' ? path.join(workspaceRoot, 'smuggler-ai-gm') : null,
        ].filter(p => p !== null);
        ...
        for (const repoPath of possiblePaths) {
          try {
            ...
            if (fs.existsSync(repoPath)) {
              ...
              foundLocalPath = true;
              console.log(`[AutomatedQualityImprover] ✅ Analyzed ${repo} at ${repoPath}`);
              console.log(`[AutomatedQualityImprover]   Language: ${projectContext.primaryLanguage}, PackageManager: ${projectContext.packageManager}, Frameworks: ${projectContext.frameworks?.join(', ') || 'none'}`);
              break;
            }
          } catch (e) {
        ...
        if (!foundLocalPath) {
          console.log(`[AutomatedQualityImprover] ⚠️  Could not find local path for ${repo}, using fallback detection`);
          ...
          if (!projectContext.primaryLanguage || projectContext.primaryLanguage === 'Unknown') {
            ...
            projectContext.packageManager = 'npm'; // Assume npm if we can't detect
          }
        }
      } catch (error) {
        console.warn(`[AutomatedQualityImprover] Could not analyze project structure: ${error.message}`);
        // Fallback to JavaScript for Node.js projects
        if (!projectContext.primaryLanguage || projectContext.primaryLanguage === 'Unknown') {
          projectContext.primaryLanguage = 'JavaScript';
          projectContext.languages = ['JavaScript'];
      ...
      while (currentQuality < targetQuality && iteration < maxIterations) {
        ...
        for (const recommendation of allRecommendations.slice(0, 5)) {
          ...
          const codeGenContext = {
            repo,
            language: projectContext.primaryLanguage || (projectContext.languages && projectContext.languages.length > 0 && projectContext.languages[0]) || (projectContext.packageManager ? 'JavaScript' : 'Unknown'),
            primaryLanguage: projectContext.primaryLanguage || (projectContext.packageManager ? 'JavaScript' : null),
            languages: projectContext.languages || (projectContext.packageManager ? ['JavaScript'] : []),
            frameworks: projectContext.frameworks || [],
            hasTests: projectContext.hasTests || false,
            hasCI: projectContext.hasCI || false,
            packageManager: projectContext.packageManager || 'npm', // Default to npm if package.json exists
            buildSystem: projectContext.buildSystem,
            scripts: projectContext.scripts || {},
            description: projectContext.description || '',
            type: projectContext.type || 'library',
          };
          ...
          const generated = codeGenerator.generateFromMappings(fileActions, codeGenContext);
