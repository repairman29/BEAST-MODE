...
class ProjectAnalyzer {
  ...
  async analyzeFiles(repoPath, maxDepth = 3, currentDepth = 0) {
    ...
    // Determine primary language (filter out numeric keys)
    const languageEntries = Object.entries(languages).filter(([key]) => isNaN(key));
    const primaryLanguage = languageEntries.length > 0
      ? languageEntries.sort((a, b) => b[1] - a[1])[0][0]
      : 'Unknown';

    return {
      languages: Object.keys(languages).filter(key => isNaN(key)), // Filter out numeric keys
      primaryLanguage,
      hasTests,
      hasCI,
    };
