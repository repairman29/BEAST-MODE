...
const progressiveEnhancer = require('./progressiveEnhancer');
...
class FileQualityScorer {
   ...
   */
  async scoreFile(filePath, content, repoContext = {}) {
    ...
    // Enhance with LLM if confidence is low (progressive enhancement)
    if (progressiveEnhancer && score < 0.7) {
      try {
        const enhanced = await progressiveEnhancer.analyzeWithEnhancement(
          content,
          filePath,
          async (code, path) => baseResult,
          { repo: repoContext.repo || 'unknown' }
        );
        return enhanced.enhanced ? enhanced : baseResult;
      } catch (error) {
        // Non-critical, return base result
        return baseResult;
      }
    }
