...
class FeatureGenerator {
  ...
  async generateFeature(repo, featureRequest, files = null, options = {}) {
    ...
    try {
      ...
      // Use LLM if available (either provider API key or custom model)
      if (useLLM && (userApiKey || customModelId)) {
        generatedFiles = await this.generateWithLLM(
          ...
          context,
          {
            ...
            userId, // Pass user ID for routing
          },
          userApiKey
        );
      } else {
   ...
   */
  async generateWithLLM(prompt, context, llmOptions, userApiKey) {
    const { provider, model, temperature, maxTokens, customModelId, userId } = llmOptions;
    
    try {
      // Use llmCodeGenerator which supports custom models via modelRouter
      const llmResponse = await llmCodeGenerator.generateCode(
        prompt,
        {
          provider,
          model,
      ...
      const code = llmResponse.code || llmResponse.content || '';
      
      if (code) {
        // Try to parse multiple files from response
        // Look for file markers like ```filename.js or // File: filename.js
        const filePattern = /(?:```|```)(?:(\w+\.\w+)|file:\s*(\S+))?\n([\s\S]*?)(?:```|```)/gi;
        let match;
        let fileIndex = 0;
        
        while ((match = filePattern.exec(code)) !== null) {
          const fileName = match[1] || match[2] || `generated-${fileIndex++}.js`;
          const fileCode = match[3] || match[0];
        ...
        // Fallback to template-based if LLM didn't return code
        console.warn('[Feature Generator] LLM response had no code, using templates');
        return await this.generateWithTemplates(context);
      }
      
      return generatedFiles;
    } catch (error) {
      console.error('[Feature Generator] LLM generation failed:', error);
