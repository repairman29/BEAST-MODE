...
const qualityExplainer = require('./qualityExplainer');
...
class QualityValidator {
  ...
  async validateImprovement(repo, generatedFiles, originalQuality = {}) {
    ...
    try {
      ...
      // Generate quality explanation using LLM (if enabled)
      if (qualityExplainer && generatedFiles.length > 0) {
        try {
          const firstFile = generatedFiles[0];
          const explanation = await qualityExplainer.explainQuality(
            validation.score,
            firstFile.code || firstFile.content || '',
            validation.issues,
            { repo, filePath: firstFile.path }
          );
